...
(function () {
  ...
  /* ---------------- board (project key) derivation ----------------
   * A board is one project's feedback. Deriving it from the hostname alone is
   * only safe when the project OWNS that hostname. On a shared/multi-tenant host
   * (GitHub Pages, Netlify, Vercel, Framer, localhost…) every project answers to
   * the SAME hostname, so a hostname-only key silently merges unrelated clients
   * into one board. On those hosts we append the directory path, which is what
   * actually distinguishes one project from another.
   * Custom domains keep the bare-hostname key — existing boards stay valid. */
  var SHARED_HOST_RE = /(^|\.)(github\.io|netlify\.app|vercel\.app|pages\.dev|web\.app|firebaseapp\.com|surge\.sh|framer\.(website|app)|onrender\.com|glitch\.me|repl\.co|ngrok(-free)?\.(io|app|dev)|trycloudflare\.com|localhost)$/i;
  function isSharedHost(h) {
    return SHARED_HOST_RE.test(h) || /^\d{1,3}(\.\d{1,3}){3}$/.test(h) || h === "localhost";
  }
  function dirPath() {
    // /abby-site/home.html -> /abby-site   ·   /abby-site/ -> /abby-site   ·   / -> ""
    var p = String(location.pathname || "/");
    if (!/\/$/.test(p)) p = p.replace(/\/[^\/]*$/, "/");   // drop the file segment
    return p.replace(/\/+$/, "").replace(/^\/+/, "");       // trim slashes both ends
  }
  function autoBoard() {
    var host = normHost(location.hostname);
    if (!isSharedHost(host)) return host;                   // custom domain — unchanged
    var h = location.host ? location.host.replace(/^www\./i, "").toLowerCase() : host; // keep :port
    var d = dirPath();
    return d ? h + "/" + d : h;
  }
  ...
  var BOARD_EXPLICIT =
    ...
    CFG.projectKey || null;
  var BOARD = BOARD_EXPLICIT || autoBoard();

  // OWNER MODE (triage UI: Fix/Park/Resolve). Open once with &owner=1 to mark this
  // browser as the owner (persists); &owner=0 clears it. Clients never see triage.
  var OWNER = false;
  try {
    var op = params.get("owner");
    if (op === "1") localStorage.setItem("wr:owner", "1");
    if (op === "0") localStorage.removeItem("wr:owner");
    OWNER = localStorage.getItem("wr:owner") === "1";
  } catch (e) { OWNER = params.get("owner") === "1"; }
  ...
  // Surface the resolved project key — the operator needs it for reviews/config.json.
  // On a shared host with no explicit key, say so loudly: the auto key is path-derived
  // and will change if the site later moves to its own domain or a different folder.
  window.__wrBoard = BOARD;
  if (BOARD_EXPLICIT) {
    console.info("[review] project key: %s (explicit)", BOARD);
  } else if (isSharedHost(normHost(location.hostname))) {
    console.warn(
      "[review] project key: %s (auto, derived from host+path on a SHARED host).\n" +
      "Set an explicit key so feedback survives a move:\n" +
      '  window.WR_CONFIG = { …, projectKey: "your-project" }', BOARD);
  } else {
    console.info("[review] project key: %s (auto, from domain)", BOARD);
  }
  ...
  var notes = cacheRead();
  var comments = [];
  var atts = [];                               // attachments metadata (photos)
  var pendingFiles = {};                        // note.id -> File[] awaiting the note's insert (in-memory only)
  var armed = false, tool = "pin";
  var markers = new Map();
  var sidebarOpen = false, collapsed = false, expandedId = null, layoutFilter = "current";
  var KIND_LABEL = { pin: "pin", box: "box", text: "highlight", draw: "drawing" };   // capture = pin+draw; box/text render as legacy
  ...
  root.innerHTML =
    ...
    '.wr-capture{position:fixed;inset:0;z-index:2147483001;cursor:crosshair;background:rgba(43,108,255,.04);touch-action:none;overscroll-behavior:contain}' +
    ...
    '.wr-pin--sel{background:#ff8a1e !important;box-shadow:0 0 0 4px rgba(255,138,30,.3),0 0 16px rgba(255,138,30,.8)}' +
    ...
    '.wr-box--sel{border-color:#ff8a1e !important;background:rgba(255,138,30,.10) !important;box-shadow:0 0 14px rgba(255,138,30,.6)}' +
    ...
    '.wr-hl--sel{background:rgba(255,150,40,.5) !important;box-shadow:0 0 10px rgba(255,138,30,.65)}' +
    ...
    '.wr-draw--sel polyline{stroke:#ff8a1e !important;filter:drop-shadow(0 0 5px rgba(255,138,30,.85))}' +
    /* dock + tool tray */
    ...
    '.wr-c--sel{border-color:#ff8a1e;box-shadow:0 0 0 3px rgba(255,138,30,.22),0 0 14px rgba(255,138,30,.28)}.wr-c--done{opacity:.72}' +
    ...
    '.wr-tag--fix{background:#ffe9d4;color:#c65f00}.wr-tag--park{background:#e9ecf3;color:#5b6373}' +
    '.wr-mini--fix{background:#ff8a1e;color:#fff;border-color:#ff8a1e}' +
    ...
    /* photos */
    '.wr-attrow{display:flex;gap:6px;align-items:center;flex-wrap:wrap;margin-top:8px}' +
    '.wr-attbtn{border:1px dashed #c7cdd8;background:#fff;border-radius:9px;padding:7px 10px;font:600 12px/1 inherit;cursor:pointer;color:#3a4152}' +
    '.wr-chipimg{position:relative;width:54px;height:54px;border-radius:8px;overflow:hidden;border:1px solid #e2e5ea}' +
    '.wr-chipimg img{width:100%;height:100%;object-fit:cover;display:block}' +
    '.wr-chipimg button{position:absolute;top:1px;right:1px;border:0;background:rgba(0,0,0,.55);color:#fff;border-radius:50%;width:17px;height:17px;font-size:10px;line-height:1;cursor:pointer}' +
    '.wr-photos{display:flex;gap:6px;flex-wrap:wrap;margin:6px 0 8px}' +
    '.wr-photos img{width:64px;height:64px;object-fit:cover;border-radius:8px;border:1px solid #e2e5ea;cursor:pointer}' +
    '.wr-upl{color:#8a93a3;font-size:12px}' +
    /* popovers */
    ...
    '.wr-ring{outline:3px solid #ffce54 !important;outline-offset:3px;border-radius:999px}' +
    '@media (max-width:600px){.wr-side{width:100vw;max-width:100vw;height:74vh;top:auto;bottom:0;border-left:0;border-top:1px solid #e6e9ef;border-radius:16px 16px 0 0}' +
      '.wr-pop textarea,.wr-pop input,.wr-replyrow input,.wr-edit textarea{font-size:16px}}' +   /* 16px stops iOS zoom-on-focus (the zoom made drags pan the page) */
    '</style>' +
  ...
  /* ---------------- dock: 📍 Pin + ✎ Draw, always visible until Hide --- */
  var pinBtn = el("button", "wr-btn wr-btn--go");
  var drawBtn = el("button", "wr-btn wr-btn--go");
  var listBtn = el("button", "wr-btn wr-btn--ghost");
  var hideBtn = el("button", "wr-btn wr-btn--ghost");
  dock.appendChild(pinBtn); dock.appendChild(drawBtn); dock.appendChild(listBtn); dock.appendChild(hideBtn);
  var dot = null;
  function renderDock() {
    pinBtn.innerHTML = (armed && tool === "pin") ? "✕ Cancel" : '<span>📍</span> Pin';
    drawBtn.innerHTML = (armed && tool === "draw") ? "✕ Cancel" : '<span>✎</span> Draw';
    if (boxBtn) boxBtn.innerHTML = (armed && tool === "box") ? "✕ Cancel" : '<span>▭</span> Box';
    if (hlBtn) hlBtn.innerHTML = (armed && tool === "text") ? "✕ Cancel" : '<span>🖍</span> Text';
    listBtn.innerHTML = 'Notes <span class="wr-badge">' + openCount() + "</span>";
    hideBtn.textContent = "👁 Hide"; hideBtn.title = "Hide review tools";
  }
  pinBtn.addEventListener("click", function () { (armed && tool === "pin") ? disarm() : arm("pin"); });
  drawBtn.addEventListener("click", function () { (armed && tool === "draw") ? disarm() : arm("draw"); });
  listBtn.addEventListener("click", function () { sidebarOpen ? closeSidebar() : openSidebar(); });
  // Owner-only extra tools: ▭ box + 🖍 text-highlight (clients get Pin + Draw only).
  var boxBtn = null, hlBtn = null;
  if (OWNER) {
    boxBtn = el("button", "wr-btn wr-btn--go");
    hlBtn = el("button", "wr-btn wr-btn--go");
    dock.insertBefore(boxBtn, listBtn); dock.insertBefore(hlBtn, listBtn);
    boxBtn.addEventListener("click", function () { (armed && tool === "box") ? disarm() : arm("box"); });
    hlBtn.addEventListener("click", function () { (armed && tool === "text") ? disarm() : arm("text"); });
  }
  ...
  /* ---------------- capture (pin + draw) ---------------- */
  var captureLayer = null, hintEl = null, live = null, prevOverflow = null, textMode = false;
  var HINTS = { pin: "Tap the spot you want to comment on", draw: "Draw on the page to point something out", box: "Drag a box around the area", text: "Select the text you want to comment on" };
  function hint(t) { var h = el("div", "wr-hint", t); wrap.appendChild(h); return h; }
  function blockTouch(e) { e.preventDefault(); }   // stops iOS panning the (possibly zoomed) page mid-gesture
  function lockScroll() {
  ...
  function arm(kind) {
    ...
    if (tool === "text") {   // selection mode: no capture layer, page stays scrollable
      textMode = true; document.addEventListener("mouseup", onTextSelect, true); return;
    }
    captureLayer = el("div", "wr-capture"); wrap.appendChild(captureLayer);
    captureLayer.addEventListener("touchmove", blockTouch, { passive: false });
    lockScroll();
  ...
  function disarm() {
    armed = false; renderDock(); unlockScroll();
  ...
  function grab(e) { try { captureLayer.setPointerCapture(e.pointerId); } catch (err) {} }
  ...
  function armBox() {
    var start = null;
    captureLayer.addEventListener("pointerdown", function (e) { e.preventDefault(); grab(e); start = { x: e.clientX, y: e.clientY }; live = el("div", "wr-rubber"); wrap.appendChild(live); setRect(live, e.clientX, e.clientY, e.clientX, e.clientY); });
    captureLayer.addEventListener("pointermove", function (e) { if (start && live) { e.preventDefault(); setRect(live, start.x, start.y, e.clientX, e.clientY); } });
    ...
    captureLayer.addEventListener("pointercancel", function () { start = null; if (live) { live.remove(); live = null; } });
  }
  function armDraw() {
    var drawing = false, pts = [], poly = null;
    captureLayer.addEventListener("pointerdown", function (e) {
      e.preventDefault(); grab(e); drawing = true; pts = [[e.clientX, e.clientY]];
    ...
    captureLayer.addEventListener("pointermove", function (e) { if (!drawing) return; e.preventDefault(); pts.push([e.clientX, e.clientY]); poly.setAttribute("points", pts.map(function (p) { return p[0] + "," + p[1]; }).join(" ")); });
    ...
    captureLayer.addEventListener("pointercancel", function () { drawing = false; if (live) { live.remove(); live = null; } });
  }
  ...
  function onTextSelect(e) {
    if (!host.isConnected) { document.removeEventListener("mouseup", onTextSelect, true); return; }   // stale instance guard
    try {
  ...
  /* ---------------- welcome + tour (first session only) ---------------- */
  var TOUR_KEY = "wr:tour";
  function markTour() { try { localStorage.setItem(TOUR_KEY, "1"); } catch (e) {} }
  function openWelcome() {
    var pop = el("div", "wr-pop"); pop.style.left = "16px"; pop.style.bottom = "70px"; pop.style.top = "auto";
    var t = el("div", null, "Welcome! You've been invited to review this site.");
    t.style.cssText = "font-weight:700;margin-bottom:4px";
    var s = el("div", null, "Leave notes right on the page — they go straight to the site builder. First, what's your name?");
    s.style.cssText = "color:#697086;font-size:12.5px;line-height:1.45;margin-bottom:9px";
    pop.appendChild(t); pop.appendChild(s);
    var inp = el("input"); inp.placeholder = "e.g. Ruth"; inp.setAttribute("dir", "auto"); pop.appendChild(inp);
    var row = el("div", "wr-row");
    var skip = el("button", "wr-mini", "Skip tour");
    var go = el("button", "wr-btn wr-btn--go", "Start tour →");
    row.appendChild(skip); row.appendChild(go); pop.appendChild(row);
    wrap.appendChild(pop); try { inp.focus(); } catch (e) {}
    function save(withTour) {
      var v = inp.value.trim(); if (!v) return inp.focus();
      ME = v; try { localStorage.setItem(LS.name, v); } catch (e) {}
      pop.remove();
      if (withTour) runTour(0); else markTour();
    }
    go.addEventListener("click", function () { save(true); });
    skip.addEventListener("click", function () { save(false); });
    inp.addEventListener("keydown", function (e) { if (e.key === "Enter") save(true); });
  }
  var TOUR = [
    { btn: function () { return pinBtn; }, title: "Pin 📍 & Draw ✎", body: "Tap Pin, then tap any spot to leave a note there. Or tap Draw and scribble right on the page. Type what should change and save." },
    { btn: function () { return listBtn; }, title: "Notes", body: "Every note lands here. Tap one to jump to its exact spot on the page, reply, or add a photo." },
    { btn: function () { return hideBtn; }, title: "Hide 👁", body: "Hides the review tools so you see the site clean. Tap the blue dot (bottom-left) to bring them back. That's it — happy reviewing!" }
  ];
  var tourRing = null;
  function clearRing() { if (tourRing) { tourRing.classList.remove("wr-ring"); tourRing = null; } }
  function runTour(i) {
    clearRing();
    var old = root.querySelector(".wr-tourpop"); if (old) old.remove();
    if (i >= TOUR.length) { markTour(); return; }
    var st = TOUR[i];
    tourRing = st.btn(); tourRing.classList.add("wr-ring");
    var pop = el("div", "wr-pop wr-tourpop"); pop.style.left = "16px"; pop.style.bottom = "70px"; pop.style.top = "auto";
    var head = el("div", "wr-row");
    head.appendChild(el("div", "wr-sp", (i + 1) + " / " + TOUR.length));
    var xb = el("button", "wr-x", "✕"); head.appendChild(xb); pop.appendChild(head);
    var t = el("div", null, st.title); t.style.cssText = "font-weight:700;margin-bottom:4px"; pop.appendChild(t);
    var b = el("div", null, st.body); b.style.cssText = "color:#697086;font-size:12.5px;line-height:1.5"; pop.appendChild(b);
    var row = el("div", "wr-row");
    var next = el("button", "wr-btn wr-btn--go", i === TOUR.length - 1 ? "Done ✓" : "Next →");
    row.appendChild(next); pop.appendChild(row);
    wrap.appendChild(pop);
    xb.addEventListener("click", function () { clearRing(); pop.remove(); markTour(); });
    next.addEventListener("click", function () { pop.remove(); runTour(i + 1); });
  }
  ...
  function openComposer(x, y, a, kind) {
    ...
    var head = el("div", "wr-row");
    head.appendChild(el("div", "wr-sp", "New " + (KIND_LABEL[kind] || "note") + " · " + (a.section_id ? "#" + a.section_id : "page") + " · " + CUR));
    var xb = el("button", "wr-x", "✕"); head.appendChild(xb); pop.appendChild(head);
    ...
    // photo attach
    var picked = [];
    var fileIn = document.createElement("input"); fileIn.type = "file"; fileIn.accept = "image/*"; fileIn.multiple = true; fileIn.style.display = "none"; pop.appendChild(fileIn);
    var attRow = el("div", "wr-attrow");
    var attBtn = el("button", "wr-attbtn", "📎 Add photo"); attRow.appendChild(attBtn); pop.appendChild(attRow);
    attBtn.addEventListener("click", function (e) { e.preventDefault(); fileIn.click(); });
    fileIn.addEventListener("change", function () {
      Array.prototype.forEach.call(fileIn.files, function (f) {
        picked.push(f);
        var chip = el("div", "wr-chipimg");
        var im = document.createElement("img"); im.src = URL.createObjectURL(f); chip.appendChild(im);
        var x2 = el("button", null, "✕"); x2.addEventListener("click", function (e2) { e2.preventDefault(); picked.splice(picked.indexOf(f), 1); chip.remove(); });
        chip.appendChild(x2); attRow.appendChild(chip);
      });
      fileIn.value = "";
    });
    ...
    function submit() {
      var body = ta.value.trim(); if (!body && !picked.length) return ta.focus();
      closePop();
      saveNote({ kind: kind, section_id: a.section_id, target_selector: a.selector, target_text: a.target_text, elem_desc: a.desc, geometry: a.geometry, viewport_w: innerWidth, viewport_h: innerHeight, body: body || "(photo)" }, picked);
      openSidebar();
    }
  ...
  function pinCls(note) { return "wr-pin" + (isDone(note) ? " wr-pin--done" : "") + (expandedId === note.id ? " wr-pin--sel" : ""); }
  function addMarker(note) {
    var m = markers.get(note.id);
    if (m) { m.note = note; m.el.firstChild.textContent = numberOf(note); m.el.className = pinCls(note); m.el.title = (note.author || "?") + ": " + (note.body || ""); placeMarker(m); return; }
    var b = el("button", pinCls(note));
    ...
    try {
      if (note.kind === "box") {
        ...
        if (rng) {
          ...
          for (var i = 0; i < rects.length; i++) { var rc = rects[i]; if (!first) first = rc; var hl = el("div", "wr-hl" + (done ? " wr-hl--done" : "") + (sel ? " wr-hl--sel" : "")); hl.style.left = rc.left + "px"; hl.style.top = rc.top + "px"; hl.style.width = rc.width + "px"; hl.style.height = rc.height + "px"; m.shape.appendChild(hl); }
        ...
        m.shape.setAttribute("class", "wr-draw" + (done ? " wr-draw--done" : "") + (sel ? " wr-draw--sel" : ""));
  ...
  function card(n) {
    ...
    if (OWNER && n.disposition === "fix") meta.appendChild(el("span", "wr-tag wr-tag--fix", "🔧 fix"));
    if (OWNER && n.disposition === "parked") meta.appendChild(el("span", "wr-tag wr-tag--park", "⏸ parked"));
    var nc = commentsOf(n.id).length; if (nc) meta.appendChild(el("span", "wr-tag", "💬 " + nc));
    var np = attsOf(n.id).length; if (np) meta.appendChild(el("span", "wr-tag", "📷 " + np));
    if (!onPage(n)) meta.appendChild(el("span", "wr-tag", n.page_url || "/"));
    main.appendChild(meta); head.appendChild(main);
    head.addEventListener("click", function () {
      var opening = !open; expandedId = opening ? n.id : null; renderSidebar(); renderAll();
      if (opening && onPage(n)) jumpTo(n);   // scroll to it on THIS page; cross-page travel only via "locate →"
    });
    c.appendChild(head);
  ...
  function cardBody(n) {
    ...
    var ph = attsOf(n.id);
    if (ph.length) {
      var pg = el("div", "wr-photos");
      ph.forEach(function (a2) {
        var im = document.createElement("img"); im.src = pubURL(a2.storage_path); im.loading = "lazy";
        im.addEventListener("click", function () { window.open(pubURL(a2.storage_path), "_blank"); });
        pg.appendChild(im);
      });
      wrapc.appendChild(pg);
    }
    if (n.resolution) { var res = el("div", "wr-anchor"); res.textContent = "Resolved: " + n.resolution; wrapc.appendChild(res); }
    if (OWNER && n.owner_note) { var on = el("div", "wr-anchor"); on.setAttribute("dir", "auto"); on.textContent = "🔧 For Claude: " + n.owner_note; wrapc.appendChild(on); }
    ...
    var rIn = document.createElement("input"); rIn.type = "file"; rIn.accept = "image/*"; rIn.multiple = true; rIn.style.display = "none"; rr.appendChild(rIn);
    var rAtt = el("button", "wr-mini", "📎"); rAtt.title = "Attach photo to this note"; rr.appendChild(rAtt);
    rAtt.addEventListener("click", function () { if (n._pending) return; rIn.click(); });
    rIn.addEventListener("change", function () {
      var fs = Array.prototype.slice.call(rIn.files); rIn.value = "";
      if (fs.length) ensureName(function () { rAtt.textContent = "…"; uploadPhotos(n, fs, function () { rAtt.textContent = "📎"; }); });
    });
    ...
    if (OWNER) {
      // Owner triage: queue for Claude (optionally with the reply box's text as the instruction) or park for later.
      var fixB = el("button", "wr-mini wr-mini--fix", n.disposition === "fix" ? "🔧 Queued ✓" : "🔧 Fix");
      fixB.addEventListener("click", function () {
        if (n.disposition === "fix") { setDisposition(n, null, null); return; }
        var instr = ri.value.trim();
        if (!instr) instr = window.prompt("Instruction for Claude (optional — empty just queues it):", "") || "";
        ri.value = "";
        setDisposition(n, "fix", instr || null);
        if (instr) addComment(n.id, "🔧 " + instr);
      });
      acts.appendChild(fixB);
      var parkB = el("button", "wr-mini", n.disposition === "parked" ? "⏸ Parked ✓" : "⏸ Park");
      parkB.addEventListener("click", function () { setDisposition(n, n.disposition === "parked" ? null : "parked", undefined); });
      acts.appendChild(parkB);
  ...
  function jumpTo(note) {
    if (!onPage(note)) {
      var u = note.page_url || "/", qs = "review=1";
      if (params.get("board")) qs += "&board=" + encodeURIComponent(params.get("board"));   // keep the board override across pages
      location.href = u + (u.indexOf("?") >= 0 ? "&" : "?") + qs;
      return;
    }
  ...
  /* ---- deep link: ?wrnote=<id> lands on the page AND goes to the pin ----
   * The dashboard's "open at this spot →" carries the note id. Fires ONCE: notes
   * re-fetch on every tab focus, and re-scrolling would yank the page out from
   * under whoever is reading it. Waits for load+settle because a pin halfway down
   * a page of unsized images is at the wrong Y until those images have height. */
  var WANT_NOTE = params.get("wrnote") || "";
  function whenSettled(fn) {
    if (document.readyState === "complete") return setTimeout(fn, 150);
    window.addEventListener("load", function () { setTimeout(fn, 150); });
  }
  function focusRequestedNote() {
    if (!WANT_NOTE) return;
    var want = WANT_NOTE, n = null;
    for (var i = 0; i < notes.length; i++) if (notes[i].id === want) { n = notes[i]; break; }
    if (!n) return;                     // not in this fetch yet — stay armed for the next one
    WANT_NOTE = "";
    whenSettled(function () {
      var a = onPage(n) ? anchorEl(n) : null;
      if (a) a.scrollIntoView({ behavior: "smooth", block: "center" });
      // No anchor (element gone, or a note taken on the other layout) => don't leave the
      // click looking dead: show the note itself in the sidebar instead.
      if (a) pulse(n.id); else openInSidebar(n);
    });
  }
  ...
  function uploadPhotos(note, files, doneCb) {
    files = (files || []).slice();
    if (!files.length) { if (doneCb) doneCb(); return; }
    if (!supa) { console.warn("[review] photos need a connection — note saved, attach again later."); if (doneCb) doneCb(); return; }
    var left = files.length;
    files.forEach(function (f) {
      prepPhoto(f).then(function (p) {
        if (p.blob.size > 10485760) throw new Error("photo over 10 MB after compression");
        var path = "projects/" + BOARD + "/" + note.id + "/" + uid() + "." + p.ext;
        return supa.storage.from("review-photos").upload(path, p.blob, { contentType: p.mime }).then(function (r) {
          if (r.error) throw r.error;
          return supa.from("attachments").insert({ note_id: note.id, project_key: BOARD, storage_path: path, mime: p.mime, bytes: p.blob.size, author: ME }).then(function (r2) {
            if (r2.error) throw r2.error;
            atts.push({ note_id: note.id, project_key: BOARD, storage_path: path, mime: p.mime, author: ME });
          });
        });
      }).catch(function (e) { console.warn("[review] photo upload failed:", e.message || e); })
        .then(function () { if (--left === 0) { renderAll(); if (sidebarOpen) renderSidebar(); if (doneCb) doneCb(); } });
    });
  }

  /* ---------------- data ops ---------------- */
  function colsOf(n) {
    var c = { id: n.id, project_key: BOARD, kind: n.kind, page_url: n.page_url, page_title: n.page_title, section_id: n.section_id, target_selector: n.target_selector, target_text: n.target_text, elem_desc: n.elem_desc, geometry: n.geometry, viewport_w: n.viewport_w, viewport_h: n.viewport_h, body: n.body, status: n.status, author: n.author, author_id: n.author_id };
    // only send triage columns when set, so inserts keep working pre-migration (fix-park.sql)
    if (n.disposition != null) { c.disposition = n.disposition; c.owner_note = n.owner_note || null; }
    return c;
  }
  function saveNote(partial, files) {
    ...
    if (files && files.length) pendingFiles[rec.id] = files;   // photos wait for the note row (FK)
  ...
  function flushOne(rec) {
    ...
    supa.from("notes").insert(colsOf(rec)).then(function (res) {
      if (res.error) { console.warn("[review] insert queued (retry on next sync):", res.error.message); return; }
      delete rec._pending; cacheWrite();
      var f = pendingFiles[rec.id];
      if (f) { delete pendingFiles[rec.id]; uploadPhotos(rec, f); }
    });
  }
  ...
  function setDisposition(note, disp, ownerNote) {
    var live2 = notes.filter(function (n) { return n.id === note.id; })[0] || note;
    live2.disposition = disp;
    if (ownerNote !== undefined) live2.owner_note = ownerNote;
    live2.updated_by = ME; cacheWrite(); renderAll(); if (sidebarOpen) renderSidebar();
    if (live2._pending) { flushOne(live2); return; }
    if (supa) supa.from("notes").update({ disposition: disp, owner_note: live2.owner_note || null, updated_by: ME }).eq("id", live2.id)
      .then(function (r) { if (r.error) console.warn("[review] fix/park needs fix-park.sql run in Supabase:", r.error.message); });
  }
  ...
  function fetchNotes() {
    ...
    supa.from("notes").select("*").eq("project_key", BOARD).order("created_at", { ascending: true }).then(function (res) {
      ...
      notes = server.concat(pending); cacheWrite(); renderAll(); flushPending(); focusRequestedNote();
    });
    ...
    supa.from("attachments").select("*").eq("project_key", BOARD).then(function (res) { if (!res.error) { atts = res.data || []; if (sidebarOpen) renderSidebar(); } });
  }
  ...
  window.__wr = { arm: arm, notes: function () { return notes; }, comments: function () { return comments; }, fetch: fetchNotes, board: BOARD, sidebar: openSidebar, tour: function () { runTour(0); }, owner: OWNER };
})();
